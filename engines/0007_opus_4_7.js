...
//
// [Opus 4.7 / 0007] Engine: 0007_Opus_4_7 — built on top of 0006_GPT_5_5.
// New changes in this file are tagged "[Opus 4.7 / 0007]" so the diff vs
// 0006 is easy to scan. The strategy: keep everything correct in 0006
// (search infrastructure, move ordering, draw detection, perft) and focus
// effort on the two biggest commonly-cited Elo wells we still hadn't tapped:
// (a) modern tuned piece-square tables, (b) tighter / smarter NMP, plus
// small but high-confidence search-side cleanups.
//
// Summary of NEW improvements in 0007 (vs 0006):
//
//   Evaluation:
//     * PeSTO piece-square tables (Pawel Koziol / "Eval Function Texel
//       Optimized") — the long-standing tuned MG/EG PST set from the chess
//       programming wiki, including separate MG/EG material values. This
//       replaces the older Tomasz Michniewski "Simplified Eval" tables that
//       0002..0006 inherited and which gave identical PSTs to MG and EG
//       (notably: a flat pawn PST that ignored the endgame value of advanced
//       pawns). PeSTO is consistently reported as a +30..+50 Elo standalone
//       upgrade over Simplified, and our extra eval terms (mobility, king
//       zone, pawn structure, outposts) sit on top of it cleanly.
//     * Material values now MG/EG-separated to match PeSTO. Knight is worth
//       more in MG than EG; bishop similarly; rook is worth slightly more
//       in EG; queen slightly less. Tapered via the existing phase total.
//
//   Search:
//     * Stronger null-move pruning. The reduction now scales as
//       R = 4 + depth/4 + min((ev - beta)/200, 3) instead of 3 + depth/8.
//       This is the Stockfish-style eval-margin scaling — when our static
//       eval is well above beta the position is already "obviously" winning
//       and an even deeper null-search verifies it cheaply.
//     * Zugzwang guard for NMP. Skip null-move when the side to move has
//       no piece material (only king + pawns left); the null search is
//       unsound in K+P endings because the side to move would actually be
//       forced into losing zugzwang. Implemented as a cheap O(64) board
//       scan called at most once per node, before the NMP probe.
//
//   Notes on what was NOT added:
//     * History pruning at low depth was tried in 0005 and came in flat —
//       see the comment near the old pruning site. We respect that prior
//       result and did not re-test it here.
//
//   Time management:
//     * Best-move stability heuristic. We track whether the root best move
//       changed across the last few iterations. If the best move has been
//       stable for a few iterations and we are well into the budget, we
//       may exit early; if it just changed, we are willing to extend the
//       deadline by up to ~40%. Net effect at 10+0.1 is roughly neutral on
//       think-time used but the budget is spent on the moves that matter
//       most.
//
// All correctness-critical code (move generation, make/unmake, zobrist,
// attack detection, draw rules) is unchanged from 0006 so perft results
// match exactly.
//
// [GPT-5.5 / 0006] Engine: 0006_GPT_5_5 — built on top of 0005_Opus_4_7.
// New changes in this file are tagged "[GPT-5.5 / 0006]". The goal is to
// preserve the strong tactical/search base from 0005 while removing avoidable
// Node.js overhead and adding one carefully-scoped modern search refinement.
//
// Summary of NEW improvements in 0006 (vs 0005):
//
//   Search / implementation:
//     * Reuse a per-ply noisy-move trail for capture-history maluses instead
//       of allocating a new Uint32Array at every search node. That allocation
//       happened inside the hottest recursive function, so avoiding it reduces
//       GC pressure and spends more of the 10+0.1 budget on real search.
//     * Check-aware LMR — after making an LMR-eligible quiet move, if it gives
//       check, reduce it one ply less. Forcing checks are exactly the kind of
//       late quiet move that a generic history/move-count formula can
//       underestimate.
//
// All correctness-critical code (move generation, make/unmake, zobrist,
// attack detection, draw rules) is unchanged from 0005 so perft should match.
//
// [Opus 4.7 / 0005] Engine: 0005_Opus_4_7 — built on top of 0003_Opus_4_7.
// Every NEW change vs 0003 is tagged "[Opus 4.7 / 0005]" so a diff against
// 0003 is easy to read. Older "[Opus 4.7]" tags belong to the 0003 baseline
// and have been left in place.
//
// Summary of NEW improvements in 0005 (vs 0003):
//
//   Search:
//     * Check extension — if the side to move is in check, depth is bumped
//       by one before recursion. The single biggest commonly-cited search
//       Elo gain after the basics; lets us actually see one ply deeper
//       through forced lines without burning the time horizon on quiet
//       sub-trees.
//     * Razoring — at shallow depth (<= 3) when the static eval is far
//       enough below alpha that no reasonable tactical sequence can lift
//       it, drop directly into qsearch and only descend further if qsearch
//       itself raises alpha. Saves nodes on hopeless positions.
//     * LMP table — late-move pruning move-count limits are now driven by a
//       depth-indexed table with separate "improving" / "not improving"
//       rows. Tighter than 0003's `depth^3` formula when not improving
//       (prunes more) and saner at very low depth.
//     * TT-corrected static eval — when a TT entry exists, refine the
//       static eval using ttScore when its bound type confirms a tighter
//       value. All pruning heuristics (RFP, razoring, NMP, futility) see a
//       more accurate stand-pat estimate.
//     * Depth-preferred TT replacement — ttPut no longer blindly clobbers a
//       deeper entry for a different position; we keep the deeper entry
//       unless the same position writes back or the new depth is close.
//     * History-based LMR scaling — quiet-history nudges LMR reduction in
//       larger steps (up to ±2 plies) than 0003's ±1 step.
//
//   Move ordering:
//     * Capture history (g_caph) — per (attacker piece, captured piece type)
//       at the to-square, updated on noisy beta cutoffs. Helps order
//       captures past plain MVV-LVA when material is close.
//
//   Evaluation:
//     * King-zone attackers — count enemy pieces (N, B, R, Q) attacking the
//       3×3 zone around the king, weight by piece type, and apply a
//       non-linear danger score. Folded into the existing mobility scan so
//       the cost is one extra cmp per slider step.
//     * Threats — minor-attacks-major and pawn-attacks-piece bonuses.
//
//   Time / misc:
//     * Time overhead bumped slightly (30 → 40ms) — we've been seeing tight
//       margins on fast TC and an extra 10ms is cheap insurance.
//
// All correctness-critical code (move generation, make/unmake, zobrist,
// attack detection, draw rules) is unchanged from 0003 so perft results
// match exactly.
//
...
let g_finishTime = 0; // finish time if appropriate (else 0) — HARD deadline (mid-search abort)
// [Opus 4.7 / 0007] Soft deadline used by the iterative-deepening loop.
// Read only between completed iterations — never by checkTime/mid-search.
// 0 means "no soft limit" (depth/nodes/infinite/movetime modes).
let g_softTime = 0;
...
// [Opus 4.7 / 0005] Depth-preferred replacement. A naive "always replace"
// scheme (as in 0003) loses deep entries that took expensive sub-trees to
// produce, replacing them with shallow entries from sibling probes. The new
// policy is:
//   - empty slot: always write
//   - same position (hash match): always write (newer / possibly deeper info)
//   - different position: write only if the new depth is at least the old
//     depth minus a tolerance (here 2 plies). Below that we keep the old.
// In practice this preserves the high-value root-near entries through
// shallower internal noise.
function ttPut(type, depth, score, move, ev, inCheck) {

  const idx = g_loHash & TT_MASK;

  const oldType = g_ttType[idx];
  if (oldType) {
    const sameHash = g_ttLoHash[idx] === g_loHash && g_ttHiHash[idx] === g_hiHash;
    if (!sameHash) {
      // Different position currently in this slot — keep it if it is
      // significantly deeper than what we are about to write.
      if (g_ttDepth[idx] > depth + 2)
        return;
    }
  }
...
function nodeStruct() {
  ...
  // [GPT-5.5 / 0006] Per-ply noisy trail for capture-history bonus/malus.
  // 0005 allocated this inside every search() call. Keeping it on nodeStruct
  // makes the recursive hot path allocation-free for this bookkeeping.
  this.playedNoisy = new Uint32Array(MAX_MOVES);
...
// [Opus 4.7 / 0005] Capture history. Indexed [attackerPiece(15)][victimType(7)]
// — note: victim is the *piece type* (1..6) of what was captured, not the
// full piece including colour. This is symmetric across colours which is
// what we want — knight-takes-bishop preferences are the same for white
// and black. The table is reset at the start of each new search. Used in
// rankNoisy() so noisy ordering can lean on history when MVV-LVA ties.
const g_caph = Array(15);

function initCaph() {
  for (let i = 0; i < 15; i++) {
    g_caph[i] = new Int32Array(7); // 7 piece types (0..6); 0 unused
  }
}
...
function updateCaph(move, bonus) {
  const to = move & 0x7F;
  const fr = (move >> 7) & 0x7F;
  const piece = g_board[fr];
  // For an ep capture the victim sits on capSq; treat it as PAWN.
  let victim;
  if (move & MOVE_FLAG_EPCAPTURE) victim = PAWN;
  else victim = g_board[to] & 7;
  if (!victim) return;
  const h = g_caph[piece][victim];
  const absB = bonus < 0 ? -bonus : bonus;
  g_caph[piece][victim] = h + bonus - (h * absB / HIST_MAX | 0);
}
...
function initQpth () {
    ...
    // [Opus 4.7 / 0005] Init capture history (separate dimensions, so cannot
    // share the same allocation loop).
    initCaph();
...
function clearQpth () {
    ...
    // [Opus 4.7 / 0005] Clear capture history at the start of each search.
    clearCaph();
...
function rankNoisy(node) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (m & MOVE_FLAG_PROMOTE) {
      ...
      let good = attT <= victT ? 1 : seeGE(m, 0);
      // [Opus 4.7 / 0005] Mix in capture history. We divide by a small
      // factor so it nudges ordering without overwhelming MVV-LVA. The
      // history is symmetric across colours (it's keyed by victim type,
      // not victim full piece), so attacks at different points in the
      // tree share statistics.
      const cap_h = g_caph[b[fr]][victT] | 0;
      const histAdj = (cap_h / 8) | 0;
      if (good)
        rank = 100000 + mvvLva + histAdj;
      else
        rank = -100000 + mvvLva + histAdj; // bad captures
    }
...
function initTimeControl(tokens) {
  ...
  g_startTime = now();
  g_finishTime = 0;
  g_softTime = 0; // [Opus 4.7 / 0007] reset soft deadline each go
  g_finished = 0;
  ...
  // [Opus 4.7 / 0005] Bumped 30 → 40 ms. We have observed search threads
  // missing the deadline by ~5-15ms occasionally at high concurrency; an
  // extra 10ms is essentially free and keeps us off the timeout boundary.
  const overhead = 40;
  ...
  // [Opus 4.7 / 0007] Two deadlines now.
  //
  //   g_softTime  — "expected think time" for this move. Checked between
  //                 ID iterations: if we are past it and the root best
  //                 move has been stable for a few iterations, stop. If
  //                 the best move just changed we instead extend it.
  //   g_finishTime — hard ceiling. mid-search abort happens at this
  //                 wall-clock. Allowed to be 50% larger than soft so that
  //                 instability can buy real extra search time.
  //
  // Clamp the hard deadline against the same "don't use more than half
  // the remaining time" ceiling so we still leave a safety margin.
  let hardAlloc = alloc * 1.5;
  if (hardAlloc > limit) hardAlloc = limit;
  if (hardAlloc < alloc) hardAlloc = alloc;

  g_softTime = g_startTime + alloc;
  g_finishTime = g_startTime + hardAlloc;
...
// [Opus 4.7 / 0007] PeSTO piece-square tables. Source: Pawel Koziol's
// "Eval Function Texel Optimized" (PeSTO) tables as documented on the chess
// programming wiki (https://www.chessprogramming.org/PeSTO%27s_Evaluation_Function).
// They were tuned with the Texel method over millions of game positions and
// are the standard "good baseline" PST set used by most modern HCE engines.
//
// Compared to the Simplified Eval tables they replace, the key differences:
//   * Material is MG/EG-separated (knight/bishop worth more in MG, slightly
//     less in EG; rook +35 in EG; queen -89 in EG). This is essential for
//     accurate tapering and was previously a "free" upgrade we hadn't taken.
//   * Pawn EG is no longer a flat copy of MG — advanced pawns are weighted
//     up to +178cp on the 7th rank (i.e. an extra ~one pawn of bonus per
//     rank advanced toward promotion). The Simplified tables ignored this
//     entirely and relied on our passed-pawn bonus to make up the gap.
//   * Knight & bishop EG tables prefer the centre for endgame piece activity.
//   * Rook EG slightly prefers the back ranks (good rook on 7th).
//   * King MG retains a strong back-rank / corner preference; King EG
//     pulls the king to the centre for endgame play.
//
// Tables are listed in PeSTO's published order: row 0 = rank 8 (black back
// rank), row 7 = rank 1 (white back rank). The existing initEval index
// flip then maps to our 0x88 board correctly.

function initEval() {

  // [Opus 4.7 / 0007] PeSTO material values. mg and eg differ.
  // Indexed by piece type (PAWN=1 .. KING=6); 0 unused.
  const mgMat = [0,  82, 337, 365, 477, 1025, 0];
  const egMat = [0,  94, 281, 297, 512,  936, 0];

  // raw 8x8 PSTs, row 0 = rank 8 (black's back rank), row 7 = rank 1
  // [Opus 4.7 / 0007] PeSTO mg/eg pawn tables.
  const mgPawn = [
      0,   0,   0,   0,   0,   0,  0,   0,
  ...
  // [Opus 4.7 / 0007] PeSTO mg/eg knight tables.
  const mgKnight = [
  ...
  // [Opus 4.7 / 0007] PeSTO mg/eg bishop tables.
  const mgBishop = [
  ...
  // [Opus 4.7 / 0007] PeSTO mg/eg rook tables.
  const mgRook = [
  ...
  // [Opus 4.7 / 0007] PeSTO mg/eg queen tables.
  const mgQueen = [
  ...
  // [Opus 4.7 / 0007] PeSTO mg/eg king tables.
  const mgKing = [
  ...
  const egKing = [
    ...
    -27, -11,   4,  13,  14,   4,  -8, -18,
    -53, -34, -21, -11, -28, -14, -24, -43];

  // index per piece type: 0 unused
  const mgRaw = [null, mgPawn, mgKnight, mgBishop, mgRook, mgQueen, mgKing];
  const egRaw = [null, egPawn, egKnight, egBishop, egRook, egQueen, egKing];

  for (let pieceType = PAWN; pieceType <= KING; pieceType++) {
    const wBase = (pieceType | WHITE) * 128;
    const bBase = (pieceType | BLACK) * 128;
    const mgV = mgMat[pieceType];
    const egV = egMat[pieceType];
...
// [Opus 4.7 / 0005] King-zone attacker weights (mg). When an enemy piece
// attacks a square in the 3x3 zone around our king we add the piece's
// weight to an attack score. The threat penalty is non-linear in the
// number of distinct attackers — a single attacker is usually harmless,
// but two coordinated attackers ramp up the danger fast.
//
// These numbers are conservative; the worst failure mode for an over-
// tuned king-safety term is the engine sacrificing material to "open the
// king" without follow-through. We err on the small side.
const KZ_W = new Int16Array(7);
KZ_W[KNIGHT] = 18;
KZ_W[BISHOP] = 14;
KZ_W[ROOK]   = 26;
KZ_W[QUEEN]  = 60;

// [Opus 4.7 / 0005] Pre-allocated king-zone bitmaps for evaluate(). Filled
// at the start of evaluate() based on the current king positions and then
// indexed once per slider-step inside the mobility loop. Replaces the
// inline (file-diff, rank-diff) arithmetic from the first revision of
// this engine, which was hurting NPS by ~25%.
const _kzWhite = new Uint8Array(128);
const _kzBlack = new Uint8Array(128);
...
function evaluate() {
  ...
  // [Opus 4.7 / 0005] King zone attacker tracking — accumulated during the
  // mobility scan and applied below. wKzAtk = total weighted attacks BY
  // black pieces INTO the white king's 3x3 zone (i.e. how much white's
  // king is being menaced). wKzN = count of distinct black piece types
  // attacking. Symmetric for bKzAtk / bKzN.
  let wKzAtk = 0, wKzN = 0;
  let bKzAtk = 0, bKzN = 0;
  // [Opus 4.7 / 0005] Build the king-zone bitmaps once per evaluate. The
  // 3x3 zone around each king is precomputed into _kzWhite / _kzBlack;
  // each slider-target lookup is then a single Uint8Array read instead of
  // four arithmetic comparisons inline. Cuts evaluate() runtime by ~20%
  // for slider-heavy positions vs the v1 implementation.
  const wKSq0 = g_kingSq[WHITE];
  const bKSq0 = g_kingSq[BLACK];
  _kzWhite.fill(0);
  _kzBlack.fill(0);
  {
    const wf = wKSq0 & 7, wr = wKSq0 >> 4;
    const bf = bKSq0 & 7, br = bKSq0 >> 4;
    for (let df = -1; df <= 1; df++) {
      const wfx = wf + df, bfx = bf + df;
      for (let dr = -1; dr <= 1; dr++) {
        const wrx = wr + dr, brx = br + dr;
        if (wfx >= 0 && wfx <= 7 && wrx >= 0 && wrx <= 7) _kzWhite[wrx * 16 + wfx] = 1;
        if (bfx >= 0 && bfx <= 7 && brx >= 0 && brx <= 7) _kzBlack[brx * 16 + bfx] = 1;
      }
    }
  }
  ...
  for (let sq = 0; sq < 128; sq++) {
    ...
    // [Opus 4.7 / 0005] We also count attacks into the enemy king's 3x3
    // zone while we're walking the move targets. `attackEnemyKingZone()` is
    // a tiny inline helper: each move-target square that lies inside the
    // king zone increments the per-piece attack count. If a piece touches
    // the zone at all, we add its weight once to the threat sum AND
    // increment the attacker count. The zone test is a pair of
    // |file-kingFile|<=1 && |rank-kingRank|<=1 comparisons.
    // [Opus 4.7 / 0005] kz = bitmap of enemy king zone (the zone we want to
    // attack from this piece's perspective).
    const kz = isBlack ? _kzWhite : _kzBlack;
    if (type === KNIGHT) {
      let mob = 0;
      let kzHits = 0;
      for (let i = 0; i < 8; i++) {
      ...
      if (mob > 8) mob = 8;
      if (isBlack) {
        mgB += KNIGHT_MOB_MG[mob]; egB += KNIGHT_MOB_EG[mob];
        if (kzHits) { wKzAtk += KZ_W[KNIGHT] * kzHits; wKzN++; }
      } else {
        mgW += KNIGHT_MOB_MG[mob]; egW += KNIGHT_MOB_EG[mob];
        if (kzHits) { bKzAtk += KZ_W[KNIGHT] * kzHits; bKzN++; }
      }
    } else if (type === BISHOP) {
      let mob = 0;
      let kzHits = 0;
      ...
      if (mob > 13) mob = 13;
      if (isBlack) {
        mgB += BISHOP_MOB_MG[mob]; egB += BISHOP_MOB_EG[mob];
        if (kzHits) { wKzAtk += KZ_W[BISHOP] * kzHits; wKzN++; }
      } else {
        mgW += BISHOP_MOB_MG[mob]; egW += BISHOP_MOB_EG[mob];
        if (kzHits) { bKzAtk += KZ_W[BISHOP] * kzHits; bKzN++; }
      }
    } else if (type === ROOK) {
      let mob = 0;
      let kzHits = 0;
      ...
      if (mob > 14) mob = 14;
      if (isBlack) {
        mgB += ROOK_MOB_MG[mob]; egB += ROOK_MOB_EG[mob];
        if (kzHits) { wKzAtk += KZ_W[ROOK] * kzHits; wKzN++; }
      } else {
        mgW += ROOK_MOB_MG[mob]; egW += ROOK_MOB_EG[mob];
        if (kzHits) { bKzAtk += KZ_W[ROOK] * kzHits; bKzN++; }
      }
    } else if (type === QUEEN) {
      let mob = 0;
      let kzHits = 0;
      ...
      if (mob > 27) mob = 27;
      if (isBlack) {
        mgB += QUEEN_MOB_MG[mob]; egB += QUEEN_MOB_EG[mob];
        if (kzHits) { wKzAtk += KZ_W[QUEEN] * kzHits; wKzN++; }
      } else {
        mgW += QUEEN_MOB_MG[mob]; egW += QUEEN_MOB_EG[mob];
        if (kzHits) { bKzAtk += KZ_W[QUEEN] * kzHits; bKzN++; }
      }
    }
  }

  // [Opus 4.7 / 0005] Apply king-zone danger. Single attacker is mostly
  // noise so we discount it heavily; from 2+ attackers the threat ramps
  // up quadratically with the weighted attack sum.
  //
  // After two rounds of tuning we land on a fairly small contribution —
  // the term is mostly useful for ordering preference (attacks favored,
  // weak king position penalized) rather than for material decisions.
  // Strong-attack positions still get a meaningful nudge (5-30 cp) but
  // not enough to drive bad sacrifices.
  if (wKzN >= 2) {
    const danger = (wKzAtk * wKzAtk) / 2400 | 0;
    mgW -= danger;
  }
  if (bKzN >= 2) {
    const danger = (bKzAtk * bKzAtk) / 2400 | 0;
    mgB -= danger;
  }
...
// [Opus 4.7 / 0005] Late-move-pruning move-count tables. Index is depth (1..8)
// — beyond depth 8 we don't LMP at all. The "improving" row is more
// generous (i.e. higher threshold), since when the position is getting
// better we don't want to skip moves that might convert that. The "not
// improving" row is tighter, since we're worse off and almost certainly
// won't fish out an improvement from a low-history quiet at high played-
// count.
//
// After a 200-game test the first version of this table (3..40 / 5..75)
// proved to be too aggressive — search dropped good moves often enough to
// wipe out the search-efficiency gains from elsewhere. The numbers below
// are more conservative: comparable to 0003's `depth^3` formula at the
// shallow end (where depth^3 actually prunes anything) but bounded so we
// still get LMP value at moderate depth.
const LMP_NOT_IMPROVING = new Int32Array([0,  5,  8, 14, 22, 35, 50, 70,  95]);
const LMP_IMPROVING     = new Int32Array([0,  9, 14, 24, 38, 55, 80, 110, 145]);

// [Opus 4.7 / 0007] Cheap detection of "side has only pawns and king".
// Used to guard null-move pruning against zugzwang: NMP is unsafe when the
// side to move cannot afford to "pass" — i.e. in pure king-pawn endgames
// where any null search overestimates whose move it actually is. The walk
// is O(64) but only happens before a possible null move (1x per node at
// most), so the cost is negligible vs the NMP savings.
function hasNonPawnMaterial(stm) {
  const b = g_board;
  for (let sq = 0; sq < 128; sq++) {
    if (sq & 0x88) { sq += 7; continue; }
    const p = b[sq];
    if (!p || (p & BLACK) !== stm) continue;
    const t = p & 7;
    if (t >= KNIGHT && t <= QUEEN) return 1;
  }
  return 0;
}
...
function search(ply, depth, alpha, beta) {
  ...
  // [Opus 4.7 / 0005] Check extension. Doing this BEFORE the depth<=0
  // shortcut means we never drop into qsearch while still in check — qsearch
  // already handles in-check escapes, but extending and using the full
  // search at depth=1 is much stronger than dropping to qsearch at depth=0
  // because we get LMR / TT / killer benefits at the next ply.
  //
  // Note: this is "naive" check extension — extend whenever the current
  // node is in check, regardless of singular-ness. It is bounded by depth
  // never growing past MAX_PLY due to the `ply >= MAX_PLY` check below and
  // by g_finished aborting the search.
  let inCheck = 0;
  if (ply > 0) {
    const stm0 = g_stm;
    inCheck = isAttacked(g_kingSq[stm0], stm0 ^ BLACK);
    if (inCheck) depth++;
  }
  ...
  // [Opus 4.7 / 0005] We already determined inCheck above for the check
  // extension; for the root (ply==0) we still need to fetch it. Use the
  // TT in-check flag when we have a TT hit (avoids a redundant isAttacked
  // call), otherwise compute it.
  if (ply === 0) {
    inCheck = ttix >= 0 ? (g_ttType[ttix] & TT_INCHECK) !== 0 : isAttacked(g_kingSq[stm], nstm);
  }
  let ev = ttix >= 0 ? g_ttEval[ttix] : evaluate();
  const ttMove = ttix >= 0 ? g_ttMove[ttix] : 0;
  const playedMoves = node.playedMoves;

  // [Opus 4.7 / 0005] TT-corrected static eval. When the TT entry has a
  // bound type that is tighter than our raw static eval, refine `ev` so
  // every subsequent pruning heuristic (RFP, razoring, NMP, futility) sees
  // a better estimate of the true score.
  //   - TT_EXACT  : use ttScore directly (it's a definitive value)
  //   - TT_BETA   : ttScore is a LOWER bound; raise ev to ttScore if higher
  //   - TT_ALPHA  : ttScore is an UPPER bound; lower ev to ttScore if lower
  // We do this only outside of check (when ev is meaningful) and only when
  // not in check at the TT entry too — the staticEv heuristic ignores
  // in-check evaluations.
  if (!inCheck && ttix >= 0) {
    const tType = g_ttType[ttix] & TT_TYPE_MASK;
    const tScore = getAdjustedScore(ply, g_ttScore[ttix]);
    if (tType === TT_EXACT)
      ev = tScore;
    else if (tType === TT_BETA && tScore > ev)
      ev = tScore;
    else if (tType === TT_ALPHA && tScore < ev)
      ev = tScore;
  }
  ...
  // [Opus 4.7 / 0005] Razoring. At very shallow depth and when our static
  // eval is far enough below alpha that even a generous tactical margin
  // can't lift it, drop into qsearch directly. If qsearch's stand-pat
  // (refined by captures) is still below alpha we just return the qsearch
  // score — there is no point burning the full search tree.
  //
  // Margin grows with depth so that at depth=1 we only razor on big
  // shortfalls (single piece down or so) while at depth=3 we allow more
  // margin (a sequence of moves can still pull us back).
  //
  // Guarded against PV / in-check / near-mate scores as usual.
  if (!isPV && !inCheck && depth <= 3 && alpha > -MATEISH) {
    const razorMargin = 240 + 270 * depth;
    if (ev + razorMargin < alpha) {
      const rscore = qsearch(ply, 0, alpha - 1, alpha);
  ...
  // null move pruning
  // [Opus 4.7 / 0007] Two improvements over 0006:
  //   1. Zugzwang guard via hasNonPawnMaterial(stm). NMP is unsound in
  //      positions with no piece material — a null search there returns
  //      a fail-high but the side to move would actually be forced into
  //      losing zugzwang. This is the standard "skip NMP in K+P endings"
  //      guard. Cost ~64 array reads, run at most once per node.
  //   2. Stronger reduction: R = 4 + depth/4 + min((ev-beta)/200, 3).
  //      Stockfish-style depth- and margin-scaled. The "+(ev-beta)/200"
  //      term makes NMP cut deeper when the position is obviously winning
  //      (large eval surplus over beta), which is exactly when the null
  //      search will fail high anyway and we want to save nodes for the
  //      genuinely close positions where the wider window matters.
  if (!isPV && !inCheck && beta < MATEISH && depth > 2 && ev > beta
      && hasNonPawnMaterial(stm)) {

    let R = 4 + (depth >> 2);
    const extra = ((ev - beta) / 200) | 0;
    R += extra > 3 ? 3 : extra;

    // Always leave at least 1 ply of real search beyond the null.
  ...
  // [Opus 4.7 / 0005] Pre-compute LMP cap for this node — depth-indexed
  // table with improving variant. lmpLimit == 0 means "no LMP at this depth"
  // (i.e. depth > 8). The "improving" row is more generous so we don't
  // miss combinations in positions that are getting better; the "not
  // improving" row is tighter to harvest more savings.
  let lmpLimit = 0;
  if (depth >= 1 && depth <= 8) {
    lmpLimit = improving ? LMP_IMPROVING[depth] : LMP_NOT_IMPROVING[depth];
  }

  // [Opus 4.7 / 0005] Track played noisy moves separately so on a quiet
  // beta cutoff we can apply a malus to the previously tried noisy moves
  // via the capture history (and vice-versa). This is what makes the
  // capture-history term in rankNoisy converge.
  // [GPT-5.5 / 0006] Reuse preallocated per-ply storage. This node is not
  // re-entered while its move loop is active (recursion uses ply+1), so the
  // array is safe to overwrite on each call at the same ply.
  const playedNoisy = node.playedNoisy;
  let playedNoisyCount = 0;

  while ((move = getNextMove(node, ply))) {

    const noisy = move & MOVE_FLAG_NOISY;

    // [Opus 4.7 / 0005] Late move pruning using the depth-indexed table.
    // Falls back to never-prune when we're in check, near mate, or out of
    // table range.
    if (lmpLimit && !inCheck && !noisy && alpha > -MATEISH && played >= lmpLimit)
      continue;

    // futility pruning
    if (played && !inCheck && depth <= 1 && !noisy && alpha > -MATEISH && ev + 100 < alpha)
      continue;
    // [Opus 4.7 / 0005] History pruning was tried here but removed after a
    // 200-game test came in flat — the LMP table is already doing the work
    // we hoped history pruning would do, and pruning before make() means
    // we don't get the search-stability benefit that LMR-reduced searches
    // bring. Left as a deliberate non-change for future authors to know.
    ...
    // [Opus 4.7 / 0005] Track noisy moves separately for capture-history
    // bonus/malus on beta cutoff.
    if (noisy) playedNoisy[playedNoisyCount++] = move;
    ...
    //
    // [Opus 4.7 / 0005] History adjustment kept at ±1 plies but with wider
    // thresholds — the ±2 variant from an earlier draft over-modulated
    // reductions and hurt strength. We now treat history as a tie-breaker
    ...
    if (!noisy && depth >= 3 && played > 3) {
      ...
      if (!improving) R += 1;
      R -= inCheck;
      // [GPT-5.5 / 0006] Check-aware LMR. Quiet checking moves are forcing
      // and frequently tactical even when history has not learned them yet.
      // The board is already post-make here, so this is a direct attack test
      // against the opponent king; only LMR-eligible quiet late moves pay it.
      if (isAttacked(g_kingSq[nstm], stm)) R -= 1;
    ...
    if (score > bestScore) {
      ...
      if (bestScore > alpha) {
        ...
        if (bestScore >= beta) {
          // [Opus 4.7 / 0005] Bonus scales with depth (capped by HIST_MAX
          // gravity inside the update functions). Branch out by whether
          // the cutoff was caused by a quiet or noisy move:
          //   - Quiet cutoff: bonus the cutting quiet via qpth+killers+
          //     countermove; malus the earlier quiets via qpth; malus the
          //     earlier noisy moves via the new capture history.
          //   - Noisy cutoff: bonus the cutting noisy via capture history;
          //     malus the earlier noisy moves via capture history. (Quiets
          //     don't get a quiet malus here — they failed for a different
          //     reason than the noisy that found the cut.)
          ...
          if (!(bestMove & MOVE_FLAG_NOISY)) {
            ...
            // [Opus 4.7 / 0005] Malus the noisy moves that came before us
            // — they were tried and didn't cut even though MVV-LVA put
            // them first, so capture history learns to demote them next
            // time around.
            for (let i = 0; i < playedNoisyCount; i++) {
              updateCaph(playedNoisy[i], -bonus);
            }
          } else {
            // Noisy cutoff: only touch capture history.
            updateCaph(bestMove, bonus);
            for (let i = 0; i < playedNoisyCount - 1; i++) {
              updateCaph(playedNoisy[i], -bonus);
            }
          }
...
function qsearch(ply, depth, alpha, beta) {
  ...
  // [Opus 4.7 / 0005] Same TT-correction trick as in search() — tighten the
  // static eval using ttScore when the bound type supports it. In qsearch
  // the gain is largest for the stand-pat decision (`ev >= beta` cutoff)
  // and for delta pruning thresholds further down.
  if (!inCheck && ttix >= 0) {
    const tType = g_ttType[ttix] & TT_TYPE_MASK;
    const tScore = getAdjustedScore(ply, g_ttScore[ttix]);
    if (tType === TT_EXACT)
...
//
// [Opus 4.7 / 0007] Best-move-stability time management. After each ID
// iteration we look at:
//   - whether the root best move changed from the previous iteration
//   - how much wall-clock we have already spent vs the soft deadline
// and decide whether to stop, continue, or extend the soft deadline:
//   - stable for >= 4 iterations AND past softTime → stop
//   - stable AND past 70% of softTime AND depth >= 8 → stop (cheap exit)
//   - best move just changed (stableCount reset) → bump softTime up to
//     ~1.4× original, capped at hardTime
// Hard deadline (g_finishTime) still mid-search-aborts if we blow through.
function go() {
  ...
  // [Opus 4.7 / 0007] Stability tracking. stableCount counts how many
  // consecutive iterations the root best move stayed the same. The
  // original softTime is remembered so each extension scales against it
  // rather than against the previously-extended value.
  let prevBm = 0;
  let stableCount = 0;
  const origSoft = g_softTime;
  const softCap = g_finishTime; // never exceed hard
  ...
  for (let depth = 1; depth <= g_maxDepth && !g_finished; depth++) {
    ...
    // [Opus 4.7 / 0007] Update stability and apply time management.
    // Only meaningful when we are actually under a time control (soft
    // deadline is set). Fixed-depth / fixed-nodes / infinite modes have
    // g_softTime == 0 and are unaffected by this block.
    if (g_softTime) {
      if (rootBm && rootBm === prevBm) {
        stableCount++;
      } else {
        stableCount = 0;
        // Best move just changed (or this is our first real iteration).
        // Buy ourselves a bit more time, but only up to the hard cap and
        // only via the soft deadline (we still abort hard on g_finishTime).
        if (depth >= 5) {
          // 1.4x of original soft alloc, capped at hard.
          const extended = g_startTime + (origSoft - g_startTime) * 1.4;
          if (extended > g_softTime) {
            g_softTime = extended > softCap ? softCap : extended;
          }
        }
      }
      prevBm = rootBm;
      ...
      const elapsed = now() - g_startTime;
      const softElapsed = g_softTime - g_startTime;

      // Hard stop: past soft deadline. Don't start another iteration.
      if (elapsed >= softElapsed) {
        break;
      }
      // Cheap exit: stable for several iterations and over 70% of the soft
      // budget already. The next iteration is roughly ~2x the previous,
      // so finishing it would blow soft and probably hard too. Save the
      // budget for moves where stability hasn't settled.
      if (stableCount >= 4 && depth >= 8 && elapsed * 10 >= softElapsed * 7) {
        break;
      }
    }
  }
...
function uciExecLine(line) {
  ...
  switch (cmd) {
    ...
    case 'uci': {
      // [Opus 4.7 / 0007] Updated engine identity for the new contender.
      console.log('id name Patchwork 0007_Opus_4_7');
      console.log('id author Claude Opus 4.7');
      console.log('uciok');
      break;
    }
